...
sem criar entidade e sem tocar no Home Assistant.
    ...
    python3 tools/sondar_fontes.py --lat -15.84 --lon -48.04 --ibge 5300108
"""
...
def buscar(url: str) -> tuple[dict | None, float, str]:
    ...
    except Exception as erro:  # sonda RELATA a falha; tratar é do coordinator
        return (
            None,
...
def main() -> int:
    ...
    if -34 <= args.lat <= 6 and -75 <= args.lon <= -33:
        ...
        if avisos:
            ...
            print(IND + f"{total} aviso(s) ativos no Brasil, {len(meus)} aqui")
            for a in meus:
                print(
                    f"            · {a['severidade']:14} {a['titulo']}  "
                    f"{a['inicio']} → {a['fim']}"
                )
            if not meus:
                print(IND + "(nenhum aviso aqui agora — resultado, não falha)")
        else:
            falhas += 1
        ...
        print(f"\n[NWS]       {estado}  {ms:.0f} ms")
        if avisos:
            print(IND + f"{len(avisos_nws.interpretar(avisos))} aviso(s) aqui")
        else:
            falhas += 1
    else:
        print(
    ...
    if args.cidade:
        ...
        if geo:
            ...
            print(IND + (geocodificacao.rotulo(achado) if achado else "não encontrada"))
        else:
            falhas += 1
